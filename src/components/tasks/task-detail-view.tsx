...
export default function TaskDetailView() {
  ...
  const [selectedPriority, setSelectedPriority] = useState<ITaskPriority | null>(null)
  const [commentText, setCommentText] = useState("")
  const [addingSubtask, setAddingSubtask] = useState(false)
  const {selectedTaskId}=useGlobalStateStore()
  ...
  useEffect(() => {
    if (selectedTaskId) {
      ...
      if (selectedTask) {
        ...
        setSelectedDate(selectedTask.dueDate ? new Date(selectedTask.dueDate) : undefined)
      }
    }
  ...
  const handleTitleSave = () => {
    if (task && title.trim()) {
      updateTask(task._id, { name: title })
  ...
  const handleDateChange = (date: Date | undefined) => {
    if (task) {
      ...
      updateTask(task._id, { dueDate: date ? date.toISOString() : null })
    }
  }
  ...
  const subtasks = task ? tasks.filter((t) => t.parentTask && t.parentTask._id === task._id) : []
      ...
      <DialogContent className="max-w-4xl p-0 h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={closeTaskModal}>
              <X className="h-4 w-4" />
            </Button>
          </div>
        </div>
          ...
          <div className="flex-1 p-6 overflow-auto">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <Select defaultValue={task.status.id} onValueChange={handleStatusChange}>
                  <SelectTrigger className="w-full">
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: task.status.color }} />
              ...
              <div>
                ...
                <Select defaultValue={task.priority?.id || "none"} onValueChange={handlePriorityChange}>
                  <SelectTrigger className="w-full">
                    <div className="flex items-center gap-2">
                      {selectedPriority && <Flag className="h-4 w-4" style={{ color: selectedPriority.color }} />}
                  ...
                  <SelectContent>
                    ...
                    {priorities.map((priority) => (
                      <SelectItem key={priority.id} value={priority.id}>
                        <div className="flex items-center gap-2">
                          <Flag className="h-4 w-4" style={{ color: priority.color }} />
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="flex items-center gap-2 w-full justify-start">
                        ...
                        <span>{selectedDate ? format(selectedDate, "PPP") : "Set date"}</span>
                      </Button>
                    </PopoverTrigger>
            ...
            <div className="mb-6">
              ...
              {addingSubtask && <SubtaskForm parentTaskId={task._id} onCancel={() => setAddingSubtask(false)} />}
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1">
                      <div className="text-xs text-gray-500">
                        ...
                        changed status from <span className="font-medium">TO DO</span> to{" "}
                        <span className="font-medium">{task.status.name}</span>
                      </div>
                      <div className="text-xs text-gray-400">{new Date(task.updatedAt).toLocaleString()}</div>
                    </div>
                  </div>
                )}
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between mt-2">
                    ...
                    <Button size="sm" variant="pink" onClick={handleCommentSubmit} disabled={!commentText.trim()}>
                      Send
                    </Button>
                  </div>
                </div>
